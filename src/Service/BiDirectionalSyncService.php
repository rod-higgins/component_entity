...
class BiDirectionalSyncService {
  ...
  /**
   * @var SdcGeneratorService
   */
  protected $sdcGenerator;

  /**
   * @var TemplateGeneratorService
   */
  protected $templateGenerator;

  /**
   * @var ReactGeneratorService
   */
  protected $reactGenerator;

  /**
   * @var FileSystemWriterService
   */
  ...
  public function handleComponentTypeUpdated($component_type) {
    ...
    $options = $this->getGenerationOptions($component_type);
    $options['overwrite'] = TRUE; // Allow overwriting for updates.
  ...
  protected function updateLibrariesYml($component_type, $options) {
    ...
    if (file_exists($libraries_file)) {
      ...
      $libraries = \Symfony\Component\Yaml\Yaml::parse($content) ?: [];
    }
    ...
    $yaml_content = \Symfony\Component\Yaml\Yaml::dump($libraries, 4, 2);
